/*-----------------------------------------*\
|  RakkPirahPlusKeyboardController.h        |
|                                           |
...
#define COLOR_BUF_SIZE              512
#define EFFECT_PAGE_LENGTH          16
#define LED_SPECIAL_EFFECT_PACKETS  0x01
...
enum
{
    RakkPirahPlus_MIN_SPEED      = 0x00,
    RakkPirahPlus_MAX_SPEED      = 0x0F,
    RakkPirahPlus_MIN_BRIGHTNESS = 0x00,
    RakkPirahPlus_MAX_BRIGHTNESS = 0x0F
};


class RakkPirahPlusKeyboardController
{
public:
    RakkPirahPlusKeyboardController(hid_device* dev_handle, const hid_device_info& info);
    ~RakkPirahPlusKeyboardController();

    std::string                 GetSerialString();
    std::string                 GetDeviceLocation();
    std::string                 GetFirmwareVersion();

    void                        UpdateMode(std::vector<mode> modes, int active_mode);
    void                        SendLEDsBuffer(std::vector<RGBColor> colors, std::vector<unsigned int> pos);
    void                        SendDirect(std::vector<RGBColor> colors, std::vector<unsigned int> pos);
    void                        SendCustom(std::vector<RGBColor> colors, std::vector<unsigned int> pos);
